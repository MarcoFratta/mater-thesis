...
This chapter establishes the foundational context and motivation for this thesis work. 
The first part provides essential background information, beginning with an introduction to the Alchemist simulation framework, 
its core concepts, and its application domains. 
The concept of Domain-Specific Languages (DSLs) is then explored, examining both \textit{external} and \textit{internal} approaches, 
with particular focus on Kotlin's capabilities for building type-safe internal DSLs. 
The discussion extends to Kotlin Symbol Processing (KSP)\footnote{\url{https://kotlinlang.org/docs/ksp-overview.html}}, 
a key technology enabling advanced \textit{metaprogramming} techniques 
that support the DSL implementation presented in this thesis.

The second part addresses the broader context of configuration management for complex systems, 
analyzing the fundamental challenges and limitations inherent in traditional configuration approaches based on 
untyped data formats. 
The examination of how these limitations manifest in practice, particularly in extensible systems like Alchemist, 
leads to a discussion of the need for type-safe configuration languages that can provide several advantages 
...
\Cref{fig:alchemist-metamodel} illustrates the relationships between the main entities of the Alchemist meta-model.
...
A key technology enabling this approach is \textit{Kotlin Symbol Processing} (KSP).
...
The detailed implementation of this mechanism will be discussed in ~\Cref{chap:implementation}.
...
Gradle, which provides a hybrid model that combines declarative DSLs with imperative logic when needed, giving the user
