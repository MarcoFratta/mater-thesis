...
\label{chap:background-motivation}
This chapter is organized into two main sections. 
The first section provides the background information for this thesis, 
while the second part explores the context and motivation behind the contribution. 
...
First, we introduce the concept of Domain-Specific Language (DSL), 
then we explore in detail the usage of Kotlin to build Domain-Specific Languages, 
and finally we present the Alchemist simulation framework.
...
the application.
Common examples include SQL\footnote{\url{https://www.w3schools.com/sql/}} for database queries,
 CSS\footnote{\url{https://www.w3schools.com/css/}} for styling web pages, 
...
Tools like Ansible\footnote{\url{https://docs.ansible.com/}} (using YAML), 
Kubernetes\footnote{\url{https://kubernetes.io/}} (using YAML/JSON), and Maven\footnote{\url{https://maven.apache.org/}} (using XML) 
...
Examples include the Gradle build system\footnote{\url{https://gradle.org/}}, Ruby on Rails' Active Record\footnote{\url{https://guides.rubyonrails.org/active_record_migrations.html}}, and many testing
...
Users get autocomplete, refactoring, and debugging capabilities \textit{for free}.
...
However, modern languages with flexible syntax features, such as Ruby\footnote{\url{https://www.ruby-lang.org/}}, Scala\footnote{\url{https://www.scala-lang.org/}},
 and Kotlin, have made it possible to 
...
that help to hide implementation details and expose a clean, declarative surface syntax.
...
The cornerstone of Kotlin DSLs is the concept of \emph{lambdas with receivers}\footnote{\url{https://kotlinlang.org/docs/lambdas.html\#function-literals-with-receiver}}.
...
\end{minted}
\captionof{lstlisting}{Example of extension function usage.}



\subsubsection{Delegated Properties Mechanism}

Kotlin's delegated properties\footnote{\url{https://kotlinlang.org/docs/delegated-properties.html}} allow developers 
to offload the logic for reading and writing a 
property to a separate helper object,
 known as the \textbf{delegate}. The syntax \texttt{val x: Type by delegate} instructs the compiler
  to generate code that redirects 
 all accessors of \texttt{x} to the \texttt{getValue} (and \texttt{setValue} for mutable properties)
  method of the delegate instance.

Moreover, Kotlin provides an interception hook called \textbf{provideDelegate}, which allows the delegate provider to inspect 
the property \textit{at the moment of its declaration}. This hook receives a \emph{KProperty} object containing metadata about the property, 
...
\end{minted}
\captionof{lstlisting}{Example of a delegated property.}
...
\end{minted}
\captionof{lstlisting}{Example of context parameter usage.}
...
\textbf{Kotlin Symbol Processing} (KSP) is a compiler plugin API 
developed by Google that provides a simplified 
...
The KSP API processes Kotlin programs \textbf{idiomatically}, understanding Kotlin-specific 
features such as extension functions, declaration-site variance, and local functions. 
It models types explicitly and provides basic type checking capabilities, including 
type equivalence and assign-compatibility. The API models Kotlin program structures 
at the symbol level according to the Kotlin grammar, making constructs like classes, 
class members, functions, and associated parameters accessible to processors, while 
lower-level control flow constructs like \texttt{if} blocks and \texttt{for} loops 
are not exposed at this abstraction level.

The working mechanism of KSP can be understood as a preprocessor framework for Kotlin programs. 
By considering KSP-based plugins as symbol processors, the data flow in a compilation 
...
Unlike full-fledged compiler plugins, KSP processors cannot modify the source code, 
treating source programs as read-only. This design decision prevents processors from 
changing language semantics, which could lead to confusing behavior, while still 
providing the flexibility to generate supporting code that enhances the developer experience.
...
\end{minted}
\captionof{lstlisting}{Example of a KSP processor function to get all member functions of a class.}


KSP is widely used in the Kotlin ecosystem for various code generation tasks. 
Popular use cases include generating database access code (Room\footnote{\url{https://developer.android.com/jetpack/androidx/releases/room}}),
 JSON serialization 
code (Moshi\footnote{\url{https://github.com/square/moshi}}) or dependency injection code (Dagger\footnote{\url{https://dagger.dev/}}).
...
What distinguishes Alchemist from domain-specific simulators is its extensible \emph{meta-model} architecture. 
...
while domain-specific functionality is encapsulated within incarnations that can be developed
 ...
 The configuration system is very extensible and allows configuring the simulation in a very flexible way, 
...
\begin{figure}[htbp]
    \makebox[\textwidth][c]{\includesvg[width=420pt, inkscapelatex=false]{figures/meta}}
    \caption{The Alchemist meta-model~\cite{alchemisttutorial}.} 
    \label{fig:alchemist-metamodel}
\end{figure}

The meta-model is organized around the following fundamental concepts, as shown in \Cref{fig:alchemist-metamodel}:

\begin{itemize}
    \item \textbf{Data representation}:
    \begin{itemize}
        \item \textbf{Molecules}: identifiers for data items, analogous to variable names in imperative programming languages.
         A molecule serves as a key that uniquely identifies a piece of information within a node.
        \item \textbf{Concentrations}: values associated with particular molecules, similar to values stored in variables. 
    ...
    \begin{itemize}
        \item \textbf{Nodes}: containers that hold molecules and reactions, representing entities within the simulated system. 
        Each node maintains local state through its molecules and executes reactions that modify this state over time.
        \item \textbf{Environments}: provide spatial services essential for modeling distributed systems. 
        ...
        \item \textbf{Linking rules}: functions that determine connectivity between nodes based on the current state of 
        the environment. 
        Each linking rule maps nodes to \emph{neighborhoods}, where a neighborhood consists of a center node and a set of 
        neighboring nodes. 
        ...
        \item \textbf{Neighborhoods}: entities composed of a center node and its set of neighboring nodes, representing the local interaction context for each node.
    \end{itemize}
    
    \item \textbf{Behavioral specification}:
    \begin{itemize}
        \item \textbf{Reactions}: define the behavior and evolution of the system. 
        Each reaction comprises three essential components:
        \begin{itemize}
            \item \textbf{Conditions}: functions that evaluate the current environment state and 
            determine whether the reaction can execute.
            If a condition does not hold, the associated reaction cannot run.
            \item \textbf{Actions}: operations that modify the environment when the reaction fires, 
            changing molecule concentrations, node positions, or other system state.
            \item \textbf{Time distribution}: determines when reactions occur based on an instantaneous rate computed 
            ...
            frequencies adapt to environmental conditions. It can model various stochastic processes, 
...
As previously noted, the framework includes several built-in \emph{incarnations}, each tailored to specific application domains.
...
Some of the built-in incarnations include the \textbf{SAPERE incarnation}, the initial stable incarnation developed within the European 
SAPERE project; the \textbf{Protelis incarnation}, which integrates the Protelis~\cite{protelis} aggregate programming language,
enabling the simulation of distributed algorithms where computations emerge from local interactions between nodes;
the \textbf{Scafi incarnation}, which supports the ScaFi~\cite{scafi} framework for field-based computing, 
providing similar capabilities for aggregate programming scenarios;
and the \textbf{Biochemistry incarnation}, which provides detailed biochemical reaction modeling capabilities.
...
\begin{itemize}
    ...
    \item \sloppypar\textbf{Aggregate programming systems}: simulating field-based computing paradigms where computations
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/cutting27.png}
        \caption{Self-organized structure~\cite{vascularmorphogenesis}.}
        \label{fig:snapshot1}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/snapshot2.png}
        \caption{Swarm coordination scenario~\cite{swarmprogramming}.}
        \label{fig:snapshot2}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/peer-to-peer-4.png}
        \caption{Peer-to-peer route planning and distribution of software updates~\cite{protelis}.}
        \label{fig:snapshot3}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/mira1-1.png}
        \caption{Situated recommendation system\footnote{\url{https://github.com/ICPS-MicroCity}}.}
        \label{fig:snapshot4}
    \end{subfigure}
    \caption{Alchemist simulation snapshots demonstrating various application domains.}
    \label{fig:alchemist-snapshots}
\end{figure}
...
applications and Spring Boot's\footnote{\url{https://docs.spring.io/spring-boot/index.html}} application properties 
and YAML configurations for 
dependency injection and application setup. For example, in a simulation framework 
...
to system specification and manipulation: the \textbf{imperative paradigm} 
and the \textbf{declarative paradigm}.
...
\begin{itemize}
    ...
    \item{\textbf{Configuration management engine}}: is responsible for determining a valid sequence 
...
Despite the architectural sophistication of declarative configuration systems,
 ...
 YAML Ain't Markup Language (YAML) have emerged as the most used data formats for expressing configuration artifacts 
...
XML, JSON and YAML are fundamentally \textbf{untyped} data formats. 
...
Although schemas can check syntax, confirming that mandatory fields exist, that values match expected basic types, and that the document 
...
grow more vertically rather than horizontally (see~\Cref{lst:examplecomplexsimulation}), where the  
...
While these languages have not achieved widespread adoption, they demonstrate
that type-safe configuration is both feasible and beneficial. Similarly, Gradle
exemplifies a hybrid approach that combines declarative DSLs with imperative
logic when needed, offering developers the flexibility to choose the most
appropriate paradigm for each task.
The need for a type-safe configuration language is motivated by the limitations of
the current YAML-based configuration system, as described in the previous sections,
 and by the requirement for a more powerful configuration mechanism that
can accommodate the extensibility of the Alchemist simulator and the complex
scenarios it enables.

In the context of this thesis, Kotlin was chosen for several reasons. 
...
or cross-language interoperability layers, ensuring seamless compatibility with existing codebases and direct access to the framework's APIs.

The choice of an internal DSL hosted in Kotlin represents a strategic balance: 
it avoids the heavy burden of maintaining a custom parser and IDE plugin while delivering a type-safe, tool-friendly configuration experience. 
Kotlin's static type system, combined with its DSL capabilities and IDE ecosystem, makes it possible to encode domain 
entities and relationships as first-class constructs that the compiler can verify. 
With this approach, type safety becomes essential for correctness rather than merely a convenience, 
providing configuration authors with compile-time diagnostics, refactoring support, and reusable libraries that can be shared across projects.
