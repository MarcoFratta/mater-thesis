...
As established in \Cref{sec:loadingphase1}, the \textit{Loader} serves as a factory for simulation instances, 
...
\begin{itemize}
    ...
    that holds all information needed to generate a simulation given the user configuration. The \textit{Launcher} is a 
...
who retains the flexibility to specify custom \textit{Launchers} without coupling the loader instance 
...
\newpage
    ...
    \item \textbf{Terminators Context}, \textit{Output Monitors Context}, and \textit{Global Programs Context} allow the addition of respective components.
    \begin{itemize}
        \item Available elements: -;
        \item Available operations: \textit{add} 
    \end{itemize}
...
\begin{enumerate}
    ...
    a \textbf{SimulationContext} instance that stores all the needed information to 
    later allow the DSL loader component to materialize the simulation model, 
    given the variable values for the current run. This \textit{SimulationContext} instance should be considered the \textit{recipe}
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{figures/variablesdesign.pdf}
