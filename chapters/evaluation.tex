...
The \texttt{VariablesContext} received the most extensive unit testing coverage due to its central 
...
To demonstrate compliance with the \textbf{YAML equivalence} requirement (\hyperref[subsec:yaml-equivalence]{FR1}), an integration test suite was developed. 
...
While the primary goal of the DSL is usability and safety, as required by the \textbf{Performance}
 non-functional requirement (\hyperref[subsec:performance]{NFR3}), the performance of the loading phase must 
...
descriptions (YAML) to developing active simulation software. 
This shift directly addresses the \textbf{Usability} non-functional requirement (\hyperref[subsec:dsl-api-usability]{NFR1}) 
by introducing several key improvements:
...
The \Cref{lst:yaml-version} and \Cref{lst:dsl-version} show a comparison between a YAML configuration and the equivalent DSL configuration:
...
\captionof{lstlisting}
{Equivalent DSL configuration of \protect\Cref{lst:yaml-version}.}
\label{lst:dsl-version}
...
The \texttt{createLagoon} function can now be used to create a lagoon deployment in any simulation script:
...
Modifying the lagoon deployment requires only updating the \texttt{createLagoon} function, 
...
For example, a \texttt{lagoon} block can be defined as follows:
...
This domain-driven customization significantly enhances the readability and expressiveness of simulation scripts, 
...
In the example below (\Cref{lst:example-verbose}), the usage of a \texttt{BrownianMove} reaction would have been very verbose, since the reaction 
...
The KSP processor directly addresses the \textbf{Extensibility} non-functional requirement (\hyperref[subsec:extensibility]{NFR2}) by enabling 
seamless integration of new components into the DSL. Users can extend the DSL with custom components simply by annotating 
them with \texttt{@AlchemistKotlinDSL}, without requiring modifications to the DSL core or the underlying framework.