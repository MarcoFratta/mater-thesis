...
Right now, users have to write configuration files in YAML.
...
Instead of using static text files, a Domain-Specific Language (DSL)-\cite{fowler2010domain} using Kotlin has been developed.
This allows users to write their configurations as if they were writing code, which brings a lot of benefits.
Checks are performed while typing, suggestions are provided by the IDE, and the chance of making mistakes is significantly reduced.
...
Section~\ref{sec:context and motivation} details the context and motivation behind this project.
Chapter~\ref{chap:background} provides background information on Alchemist, including its application domains and use cases.
Chapter~\ref{chap:analysis} analyzes the requirements and constraints for developing the DSL,
...
Chapter~\ref{chap:evaluation} evaluates the DSL through tests and comparison with YAML configurations. 
...
than generic data formats. Prominent examples include Docker's Dockerfile syntax-\cite{dockerfile}
for container image construction, Kubernetes manifests-\cite{k8s}
for orchestrating containerized 
applications, Spring Boot's application properties and YAML configurations for 
dependency injection and application setup, Gradle's build configuration DSL 
...
critical to achieve the intended outcome. Shell scripts (citation here), 
deployment automation tools like Ansible (citation here) in its command-based mode 
...
Tools such as Docker Compose, Puppet-\cite{puppet}, Terraform-\cite{terraform} and Kubernetes manifests 
...
Chef-\cite{chef} followed a similar philosophy while embedding configuration logic within a
...
Other simulation platforms, such as Eclipse MOSAIC-\cite{mosaic} for connected mobility simulations
and AWS SimSpace Weaver-\cite{aws} for large-scale spatial simulations, similarly utilize
...
names to actual Java (citation here) or Kotlin (citation here) classes at runtime, 
...
\begin{itemize}
    ...
    \item \textbf{No type inference or IDE support}: Since class names are strings, Integrated Development Environments (IDEs) (citation here)
...
configuration files-\cite{beans},
...
The lack of type information severely limits the capabilities of Integrated Development Environments (IDEs)
and text editors.
...
This absence of IDE support increases the learning curve for new users and reduces productivity even
...
JSON Schema-\cite{jsonschema} provides a standardized way to define the expected structure of JSON documents, 
while XML Schema Definition (XSD)-\cite{xmlschema} offers similar capabilities for XML. 
YAML configurations can leverage JSON Schema or
YAML-specific schema definitions to validate document structure, required fields, and basic type constraints.

However, schema validation provides only partial solutions to the fundamental limitations 
of untyped configuration formats.
While schemas can validate \emph{syntax}, ensuring that required fields are present,
 that values conform to expected primitive types,
and that the document structure matches a defined pattern,
 they provide limited support for \emph{domain logic} and \emph{semantic validation}. 
...
to reduce duplication such as YAML that supports anchors and aliases-\cite{yamlanchor}
that allow defining 
...
such as XInclude-\cite{xmlxinclude}, YAML and JSON have no such standard capabilities. 
...
grow more vertically rather then horizontally, (see listing~\ref{lst:examplecomplexsimulation}), where the  
...
\begin{center}
    ...
    \end{minipage}
    \captionof{lstlisting}
    {Example of a complex Alchemist simulation configuration from the
     \protect\cite{domini2025fbfl} paper}
...
The YAML parser will accept this configuration as valid, since \texttt{NonExistentClass} 
...
Configuration tools like CUE-\cite{cue} unifies data, schemas, and policies inside a single constraint system, 
enabling incremental validation,
order-independent unification, and reusable templates that reduce boilerplate.
Dhall-\cite{dhall} delivers a total, strongly typed language whose expressions normalize 
...
The project originated in 2010 as part of the European SAPERE
 project\footnote{Self-Aware Pervasive Service Ecosystems}-\cite{sapere}, 
...
while domain-specific functionality is encapsulated within incarnations that can be developed
 ...
 following a YAML specification \cite{yamlspecification}.
...
\begin{figure}[htbp]
\centering
\includesvg{figures/alchemist-metamodel}
\caption{The Alchemist meta-model}
...
Each incarnation defines type systems for concentrations and specialize the framework for specific domains.
...
Some of the built-in incarnations are:
    ...
    reactions that 
    consume matching tuples and produce new ones, creating ecosystem-like behaviors-\cite{alchemistsapere}.
...
\begin{itemize}
    ...
    studied through simulation. (\cite{turin})
...
These diverse applications demonstrate Alchemist's capability to model complex systems
scenarios where spatial distribution,
temporal dynamics, and stochastic interactions play crucial roles. 

\subsection{Domain Specific Languages}
\label{subsec:dsl}

A Domain-Specific Language (DSL) is a computer programming language of limited expressiveness focused on a particular domain~\cite{fowler2010domain}.
Unlike General-Purpose Languages (GPLs) such as Java, C++, or Kotlin, which are designed to be applicable across a broad range of application domains, DSLs are specialized tools tailored to a specific problem space.
The fundamental premise of a DSL is to provide a notation that allows domain experts to express solutions in terms of domain concepts rather than programming constructs.
By raising the level of abstraction, DSLs enable users to focus on \textit{what} to compute rather than \textit{how} to compute it, bridging the semantic gap between the problem domain and the implementation~\cite{vandeursen2000dsl}.
...
Because the language constructs align directly with domain concepts, programs written in a DSL are typically more concise and self-documenting than their GPL equivalents.
This conciseness reduces the cognitive load on developers and minimizes the likelihood of lower-level implementation errors.
Second, DSLs facilitate better \textbf{communication} between software developers and domain experts.
A well-designed DSL can serve as a common language that both groups can understand, allowing domain experts to validate or even directly author the logic that drives the system.
However, these benefits come with trade-offs.
Designing and implementing a DSL requires significant effort and specialized skills.
There is also a learning curve associated with any new language, and the tooling support (IDEs, debuggers, testing frameworks) for a custom DSL typically lags behind that of established GPLs unless significant investment is made~\cite{mernik2005when}.

DSLs are generally classified into two primary categories based on their implementation approach: \textbf{external} and \textbf{internal} DSLs.

\subsubsection{External DSLs}

An external DSL is a language that has its own custom syntax and is parsed independently of the host language used to implement 
the application.
Common examples include SQL for database queries, CSS for styling web pages, and Regular Expressions for text matching.
Because external DSLs are not constrained by the syntax of an existing programming language, their designers have complete 
freedom to define a notation that best fits the domain.
This allows for highly expressive and readable syntaxes that can be optimized for domain experts who may not be programmers.

A prevalent subset of external DSLs adopts standard data serialization formats, such as XML, JSON, or YAML, as their concrete syntax.
In this approach, the DSL designer defines a schema and a set of semantic rules that interpret the data structure as executable logic.
This strategy eliminates the need to implement a custom lexer and parser, as robust libraries for these formats are available in virtually all programming environments.
Tools like Ansible (using YAML), Kubernetes (using YAML/JSON), and Maven (using XML) exemplify this pattern.
However, relying on a generic data format imposes significant constraints: the DSL is limited to the syntactic primitives of the format (e.g., key-value pairs, lists, scalars), often lacking native support for control flow, variables, or type checking.
As discussed in the context of Alchemist's current configuration, this can lead to verbose files and runtime errors that are
difficult to diagnose~\cite{fowler2010domain}.
...
An internal DSL (also known as an embedded DSL) is implemented within an existing host General-Purpose Language.
It leverages the syntax and semantics of the host language to create a specialized API that feels like a custom language.
Examples include the Gradle build system (based on Groovy and Kotlin), Ruby on Rails' Active Record, and many testing
 frameworks like ScalaTest or Kotest.
...
Users get autocomplete, refactoring, and debugging capabilities "for free".
...
create internal DSLs that are remarkably clean and expressive~\cite{fowler2010domain}.
...
and specific language features designed to support type-safe builders~\cite{kotlininaction}.
...
Prominent examples include the Gradle Kotlin DSL~\cite{gradleKotlinDSL} for build automation, 
Ktor~\cite{ktorDSL} for asynchronous web applications, and Jetpack Compose for Android UI development.
...
The power of Kotlin for DSL construction rests on several key language mechanisms that work in concert to hide implementation details and expose a clean, declarative surface syntax.

\subsubsection{Lambdas with Receivers}

The cornerstone of Kotlin DSLs is the concept of \textit{lambdas with receivers}.
...
\end{minted}
\captionof{lstlisting}{Example of a simple HTML builder DSL using lambdas with receivers}

In this example, the \texttt{head} and \texttt{body} functions are members of the \texttt{HTML} class, 
but they appear as top-level keywords within the \texttt{html} block.
...
By combining lambdas with receivers and the type system, Kotlin enables \textit{type-safe builders}~\cite{typeSafeBuilders}.
Unlike dynamic builders in languages like Groovy, Kotlin's builders enforce strict scope control.
...
If the lambda is the \textit{only} argument, the parentheses can be omitted entirely.
...
\end{minted}
\captionof{lstlisting}{Example of trailing lambda syntax making the function call resemble a control structure}
...
from them or using design patterns like Decorator.
...
\subsubsection{Context Receivers and Context Parameters}

A recent evolution in Kotlin's design is the introduction of \textit{context parameters}~\cite{contextParameters}.
These features allow functions to declare dependencies on multiple contexts simultaneously, 
...
A key technology enabling this approach is \textit{Kotlin Symbol Processing} (KSP)~\cite{kspdocs}.

KSP is a compiler plugin API developed by Google that provides a simplified and powerful way to analyze Kotlin code at compile time.
Unlike traditional Java annotation processors (KAPT), which are tied to the Java programming model and require 
generating intermediate Java stubs, KSP is designed specifically for Kotlin.
It allows build tools to parse Kotlin source code,
 inspect symbols (such as classes, functions, properties, and annotations), and generate new Kotlin code based on this analysis.
