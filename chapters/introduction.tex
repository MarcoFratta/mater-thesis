...
\begin{figure}[htbp]
    \makebox[\textwidth][c]{\includesvg[inkscapelatex=false, width=420pt]{figures/meta}}
...
Examples include the Gradle build system, Ruby on Rails' Active Record-\cite{rubyrecords}, and many testing
 frameworks like ScalaTest-\cite{scalatest} or Kotest-\cite{kotest}.
...
and specific language features designed to support type-safe builders~\cite{kotlintypesafebuilders}.
...
Ktor~\cite{ktor} for asynchronous web applications, and Jetpack Compose~\cite{jetpackcompose} for Android UI development.
...
The cornerstone of Kotlin DSLs is the concept of \textit{lambdas with receivers}-\cite{kotlinlambdareceivers}.
...
If the lambda is the \textit{only} argument, the parentheses can be omitted entirely-\cite{kotlintrailing}.
...
from them or using design patterns like Decorator-\cite{kotlinextension}.
...
A recent evolution in Kotlin's design is the introduction of \textit{context parameters}~\cite{kotlincontextparameters}.
...
A key technology enabling this approach is \textit{Kotlin Symbol Processing} (KSP)~\cite{ksp}.
