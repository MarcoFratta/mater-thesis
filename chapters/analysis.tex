...
The user can leverage the IDE to focus on the simulation configuration rather than on the language syntax.
...
than their YAML equivalents. However, the DSL should have the same semantics as the YAML configuration, 
...
even after the DSL is adopted. The Kotlin DSL should be an \textbf{alternative} to the YAML configuration format, not a replacement.
...
Currently, the loading system is able to load the simulation model from YAML configuration files with a \textit{.yaml} extension.
...
using the \textit{getDefault()} method, which will assign to each variable its default value, or by using the  
\textit{getWith()} method, which will assign to each variable the value provided in the map.
...
For each batch run, the getWith() method of the loader must be invoked.
