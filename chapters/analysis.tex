...
Domain-Specific Language for Alchemist simulation configuration. 
...
limitations imposed by the Kotlin language.
...
The user can leverage the IDE to focus on the simulation 
configuration rather than on the language syntax.
...
than their YAML equivalents. Moreover, the DSL 
...
The next sections will analyze, in more detail, the constraints and functional requirements that the DSL must satisfy.
...
\begin{itemize}
    \item \textbf{Backward compatibility}: ensuring that the DSL integrates with Alchemist's 
...
existing configuration system without breaking existing functionality. 
This constraint is crucial to ensure that the YAML configuration files can still be
 used to configure simulations, 
even after the DSL is adopted. 
The Kotlin DSL should be an \textbf{alternative} to the YAML configuration format,
 not a replacement.
...
The existing system should be extended but not replaced. 
...
However, compared to languages like Scala, Kotlin imposes several constraints 
that affect DSL design and expressiveness.
...
\begin{enumerate}
    \item \textbf{YAML Equivalence}: the DSL should be able to express all 
    simulation configurations currently
    representable in YAML and produce \textbf{identical} simulation models.
    \label{fr:yaml-equivalence}
    \item \textbf{Loading system integration}: 
    the existing loading infrastructure must be extended to recognize and process \textit{.kts} files 
    alongside \textit{.yml} files, supporting both Gradle-based and standalone execution modes;
    \label{fr:loading-integration}
    \item \textbf{Type safety}: the DSL should leverage Kotlin's type system benefits 
    to provide the IDE support and the compile-time validation.
    \label{fr:type-safety}
\end{enumerate}

These three functional requirements will be analyzed in more detail in the next sections.

\subsection{YAML Equivalence}
\label{subsec:yaml-equivalence}
The YAML equivalence functional requirement ensures that the DSL can express 
all simulation configurations currently
...
The Alchemist YAML specification can be used as a reference to define the semantics of the DSL, because we want 
the DSL to have the same semantics as the YAML configuration. 
In theory, the user should be able to convert an existing YAML configuration file to a DSL configuration file and vice versa, 
...
\label{subsec:loading-system-requirements}
...
Currently, the loading system is able to load the simulation model from YAML configuration files with a \textit{.yml} extension.
The loading system should be extended to also support the new Kotlin DSL configuration files with a \textit{.kts} extension, 
both when Alchemist is executed via Gradle build scripts and when it is run as a standalone application.
...
For a Domain-Specific Language, non-functional requirements play a crucial role 
in determining its adoption and success. 
...
Three primary non-functional requirements govern the DSL design:
\begin{enumerate}
    \item \textbf{DSL API Usability}: the DSL should provide an intuitive, readable syntax 
    that aligns with the Alchemist domain model and maximizes IDE tooling support;
    \label{nfr:usability}
    \item \textbf{Extensibility}: the DSL must support both existing and future extensions 
    without requiring modifications to the DSL core, enabling modularity and integration of new functionalities;
    \label{nfr:extensibility}
    \item \textbf{Performance}: the loading module should maintain efficiency levels comparable 
    to the existing YAML-based system.
...
\label{subsec:dsl-api-usability}
...
using familiar concepts such as \emph{nodes}, \emph{molecules}, \emph{reactions}, 
and \emph{environments} as first-class syntax elements.
...
Extensibility is another fundamental quality attribute for the Alchemist DSL, 
given the framework's nature as a general-purpose simulator.
Alchemist is designed to be extended: users can define new \emph{incarnations}, 
implement custom \emph{reactions}, create specialized \emph{environments}, 
...
As analyzed in \Cref{chap:introduction}, YAML's reuse mechanisms are limited to file-local anchors,
...
a new \emph{Incarnation} or a custom \emph{TimeDistribution}, 
...
of the \textbf{loading module} of Alchemist, which is the module responsible for the parsing of the configuration 
...
Crucially, to satisfy the \textbf{Extensibility} non-functional requirement (\hyperref[subsec:extensibility]{NFR2}), 
this mechanism cannot be hardcoded for a fixed set of classes.
...
\begin{enumerate}
    ...
    \item \textbf{Performance}: as stated earlier, while the loading time should be reasonable, 
...
\subsection{Existing Loading System}
...
\subsubsection{Loader Construction Phase}
\label{sec:loadingphase1}


\begin{figure}
    \makebox[\textwidth][c]{\includegraphics[width=\textwidth]{figures/loadingphase1.pdf}}
    \caption{The Loader Creation phase diagram illustrating the process of parsing configuration
     files into an internal map-based representation, to produce a
     reusable Loader template that can be used to generate multiple simulations.}
...
A \emph{Loader} is an object that encapsulates the configuration template along with metadata
...
\begin{itemize}
    ...
    the Alchemist simulator enables batch execution, allowing users to run the same simulation configuration multiple 
    ...
    the user can define a single configuration template where these values are
     declared as \emph{variables}.
...
\inputminted[fontsize=\footnotesize, linenos, ignorelexererrors = true]{kotlin}{listings/loader.kt}
\captionof{lstlisting}{Loader interface.}

This loader serves as a factory for generating simulation instances. A simulation can be generated either by
using the \texttt{getDefault()} method, which will assign to each variable its default value, or by using the  
\texttt{getWith()} method, which will assign to each variable the value provided in the map.

\subsubsection{Simulation Instantiation Phase}
\label{sec:loadingphase2}



This phase occurs when \texttt{getDefault()} or \texttt{getWith()} is invoked with a map of variable values, which
represents the actual parameter values for that simulation instance.
For each batch run, the \texttt{getWith()} method of the loader must be invoked.
This phase transforms the configuration template into an executable simulation model through the following steps:

\begin{figure}[htbp]
    \makebox[\textwidth][c]{\includegraphics[width=0.6\textwidth]{figures/loadingphase2.pdf}}
    \caption{The Simulation Instantiation phase diagram illustrating the transformation 
    of the configuration template into an executable simulation model.}
...
\begin{itemize}
    \item \textbf{T} (\textit{Concentration Type}): represents the type of data stored inside the nodes (molecules' concentration).
    ...
    \item \textbf{P} (\textit{Position Type}): represents the type of spatial coordinates used in the environment.
...
across all components: the \emph{Environment}, \emph{Nodes}, \emph{Reactions}, and \emph{Deployments} 
...
The crucial limitation lies in the class definitions themselves: rather than being generic classes 
that propagate the \texttt{T} and \texttt{P} types (e.g., \texttt{class Grid<P> : Deployment<P>}),
 many legacy components are non-generic and implement interfaces with \textbf{star-projections} 
 (e.g., \texttt{Deployment<Position<*>>}).
...
\end{minted}
\captionof{lstlisting}{Example of a star-projected interface.}

Passing a specific \texttt{Environment<T, P>} to this constructor is 
allowed because \texttt{Environment} is covariant (or the wildcard \texttt{<*, *>} 
accepts any projection). The fundamental issue arises from the interface implemented by the class: \texttt{Deployment<Position<*>>}.
...
the types are determined by the internal \emph{incarnation} and \emph{environment}. 
...
While the Alchemist core meta-model (described in \Cref{sec:background}) defines the runtime entities of a simulation, the \textbf{configuration domain} focuses on the declarative structures
...
Alchemist supports batch execution, allowing users to define a single configuration file 
that produces multiple simulation runs by varying parameters.
As described in \Cref{sec:loadingphase1}, the configuration domain distinguishes between three types of values: 
Constants, Free Variables and Dependent Variables.
...
\begin{itemize}
    \item \textbf{Incarnation}: the foundational factory that defines the type of the concentration $T$.
